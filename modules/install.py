...
def create_instance(server, image, log_file, storage_host="", storage_guest=""):
    ...
    if server == "kong":  # separate storage needed cases
        ...
        cmd = "docker run -d -P --net=mynet --hostname={0} -v {2}:{3} -v /data/logs/rabbitmq/rmq:/var/log/rabbitmq -v /data/logs/rabbitmq/supervisor:/var/log/supervisor --cap-add=NET_ADMIN --name={0} {1}".\
            format(server, image, storage_host, storage_guest)
